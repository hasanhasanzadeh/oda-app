...
@section('content')
    <div class="bg-white my-6 border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
        <div class="block md:flex items-start justify-between p-6 border-b lg:flex-row lg:items-center dark:border-gray-700">
            <div>
                <h2 class="text-lg font-bold text-gray-800 dark:text-white">کشورهای اخیر</h2>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">لیست آخرین کشورهای ثبت شده در سیستم</p>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tr class="bg-gray-50 dark:bg-gray-900 text-right">
                    ...
                    <x-sortable-column column="country_name" label="{{__('message.country_name')}}" :sort="$sort"
                                       :direction="$direction"/>
                    <x-sortable-column column="country_persian_name" label="{{__('message.country_persian_name')}}"
                                       :sort="$sort" :direction="$direction"/>
                    <x-sortable-column column="country_code" label="{{__('message.country_code')}}" :sort="$sort"
                                       :direction="$direction"/>
                ...
                @foreach($countries as $index=>$country)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 hover:cursor-pointer">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="mr-2 font-medium text-gray-900 dark:text-white">{{ $countries->firstItem() + $index }}</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                @endif
                                <div>
                                    <div class="text-md font-medium text-gray-900 dark:text-white">{{$country->country_name}}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-md font-medium text-gray-900 dark:text-white">
                            {{$country->country_persian_name}}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-md font-medium text-gray-900 dark:text-white">
                            ...
                            <div class="flex items-center space-x-3 space-x-reverse">
                                ...
                                <div>
                                    <form action="{{route('countries.destroy',$country->id)}}" method="post"
                                        ...
                                        @csrf
                                        {{method_field('DELETE')}}
                                        <button type="button" name="delete" onclick="confirmDelete({{ $country->id }})"
        ...
        <div class="flex flex-col md:flex-row items-center justify-between px-6 py-4 bg-white border-t dark:bg-gray-800 dark:border-gray-700">
            ...
            <div class="flex items-center justify-start w-full md:w-auto mb-4 md:mb-0 text-sm text-gray-600 dark:text-gray-400">
                <div>
                    نمایش {{ $countries->firstItem() }} تا {{ $countries->lastItem() }} از {{ $countries->total() }}
                    مورد
                </div>
            ...
            <div class="flex items-center space-x-1 space-x-reverse">
                <!-- Previous Page Link -->
                <a href="{{ $countries->appends(Request::except('page'))->previousPageUrl() }}"
                   class="{{ $countries->onFirstPage() ? 'text-gray-400 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-100' }} px-3 py-2 rounded-md">
                ...
                @if ($startPage > 1)
                    <a href="{{ $countries->appends(Request::except('page'))->url(1) }}"
                ...
                @foreach (range($startPage, $endPage) as $page)
                    <a href="{{ $countries->appends(Request::except('page'))->url($page) }}"
                ...
                @if ($endPage < $lastPage)
                    @if ($endPage < $lastPage - 1)
                        <span class="px-2 py-2 text-gray-600">...</span>
                    @endif
                    <a href="{{ $countries->appends(Request::except('page'))->url($lastPage) }}"
                       class="px-3 py-2 text-gray-600 bg-white hover:bg-gray-100 rounded-md">
                        {{ $lastPage }}
                    </a>
                @endif
                ...
                <!-- Next Page Link -->
                <a href="{{ $countries->appends(Request::except('page'))->nextPageUrl() }}"
                   class="{{ !$countries->hasMorePages() ? 'text-gray-400 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-100' }} px-3 py-2 rounded-md">
