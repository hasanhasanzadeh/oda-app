        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden mb-8">
            <div class="border-b border-gray-200 dark:border-gray-700 px-6 py-4">
                <h2 class="text-lg font-medium text-gray-700 dark:text-gray-200">
                    {{ __('message.edit') }}: {{ $page->title }}
                </h2>
            </div>

            <form method="post" action="{{ route('pages.update', $page->id) }}" enctype="multipart/form-data" class="px-6 py-4">
                @csrf
                {{ method_field('PATCH') }}
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        >
                            <option value="1" {{ $page->status == 1 ? 'selected' : '' }}>{{ __('message.active') }}</option>
                            <option value="0" {{ $page->status == 0 ? 'selected' : '' }}>{{ __('message.inactive') }}</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    <x-file-previewer name="image"
                                      ...
                                      :existing-url="$page->photo->address??''"
                                      :existing-type="$page->photo->type??''"
                    />
                </div>
