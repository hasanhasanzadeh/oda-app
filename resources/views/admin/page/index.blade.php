...
@section('content')
    <div class="bg-white my-6 border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
        <div class="block md:flex items-start justify-between p-6 border-b lg:flex-row lg:items-center dark:border-gray-700">
            <div>
                <h2 class="text-lg font-bold text-gray-800 dark:text-white">صفحات  </h2>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">لیست آخرین اطلاعات صفحات ثبت شده در سیستم</p>
            </div>
            ...
            <form method="get" id="myForm" class="block md:flex items-center mt-4 gap-4 space-x-reverse lg:mt-0">
                ...
                @endif
                <a href="{{route('pages.create')}}" class="block md:flex w-full my-4 items-center px-3 py-2 text-sm text-blue-600 bg-blue-100 rounded-lg dark:bg-blue-700 dark:text-blue-300">
                ...
                <div class="relative">
                    ...
                    <input type="text" id="searchInput" name="search" value="{{request('search')??''}}" placeholder="جستجو در صفحات ..." class="w-full md:w-64 py-2 pr-10 text-sm text-gray-700 placeholder-gray-400 bg-gray-100 border-none rounded-lg dark:bg-gray-700 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
                ...
                @foreach($pages as $index=>$page)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 hover:cursor-pointer">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <input type="checkbox" name="blog_{{$page->id}}" class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600">
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="mr-2 font-medium text-gray-900 dark:text-white">{{ $pages->firstItem() + $index }}</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <img src="{{$page->photo->address??asset('images/no-image.png')}}" class="w-20 h-20 ml-3 rounded shadow" alt="">
                                <div>
                                    <div class="text-md font-medium text-gray-900 dark:text-white">{{$page->title}}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900 dark:text-white">{{verta($page->created_at)->format('d F Y')}}</div>
                            <div class="text-xs text-gray-500 dark:text-gray-400">{{verta($page->created_at)->format('h:i A')}}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
        ...
        <div class="flex flex-col md:flex-row items-center justify-between px-6 py-4 bg-white border-t dark:bg-gray-800 dark:border-gray-700">
            ...
            <div class="flex items-center space-x-1 space-x-reverse">
                <!-- Previous Page Link -->
                <a href="{{ $pages->appends(Request::except('page'))->previousPageUrl() }}" class="{{ $pages->onFirstPage() ? 'text-gray-400 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-100' }} px-3 py-2 rounded-md">
                ...
                @if ($startPage > 1)
                    <a href="{{ $pages->appends(Request::except('page'))->url(1) }}" class="px-3 py-2 text-gray-600 bg-white hover:bg-gray-100 rounded-md">
                        1
                    </a>
                    @if ($startPage > 2)
                ...
                @foreach (range($startPage, $endPage) as $page)
                    <a href="{{ $pages->appends(Request::except('page'))->url($page) }}"
                ...
                @if ($endPage < $lastPage)
                    @if ($endPage < $lastPage - 1)
                        <span class="px-2 py-2 text-gray-600">...</span>
                    @endif
                    <a href="{{ $pages->appends(Request::except('page'))->url($lastPage) }}" class="px-3 py-2 text-gray-600 bg-white hover:bg-gray-100 rounded-md">
                        {{ $lastPage }}
                    </a>
                @endif

                <!-- Next Page Link -->
                <a href="{{ $pages->appends(Request::except('page'))->nextPageUrl() }}" class="{{ !$pages->hasMorePages() ? 'text-gray-400 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-100' }} px-3 py-2 rounded-md">
