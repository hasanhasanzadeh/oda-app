...
@section('content')
    <div class="bg-white my-6 border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
        <div class="block md:flex items-start justify-between p-6 border-b lg:flex-row lg:items-center dark:border-gray-700">
            <div>
                <h2 class="text-lg font-bold text-gray-800 dark:text-white">استان های اخیر</h2>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">لیست آخرین استان های ثبت شده در سیستم</p>
            </div>
                ...
                @foreach($provinces as $index=>$province)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 hover:cursor-pointer">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <input type="checkbox" name="city_{{$province->id}}"
                                   class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600">
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="mr-2 font-medium text-gray-900 dark:text-white">{{ $provinces->firstItem() + $index }}</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <div>
                                    <div class="text-md font-medium text-gray-900 dark:text-white">{{$province->country->country_persian_name}}</div>
                                </div>
                            </div>
                        </td>
                            ...
                            <div class="flex items-center space-x-3 space-x-reverse">
                                ...
                                <div>
                                    <form action="{{route('provinces.destroy',$province->id)}}" method="post"
                                        ...
                                        @csrf
                                        {{method_field('DELETE')}}
                                        <button type="button" name="delete" onclick="confirmDelete({{ $province->id }})"
        ...
        <div class="flex flex-col md:flex-row items-center justify-between px-6 py-4 bg-white border-t dark:bg-gray-800 dark:border-gray-700">
            ...
            <div class="flex items-center justify-start w-full md:w-auto mb-4 md:mb-0 text-sm text-gray-600 dark:text-gray-400">
                <div>
                    نمایش {{ $provinces->firstItem() }} تا {{ $provinces->lastItem() }} از {{ $provinces->total() }}
                    مورد
                </div>
                <div class="flex justify-center items-center p-3">
                    <form method="GET" action="{{ route('provinces.index') }}"
            ...
            <div class="flex items-center space-x-1 space-x-reverse">
                <!-- Previous Page Link -->
                <a href="{{ $provinces->appends(Request::except('page'))->previousPageUrl() }}"
                   class="{{ $provinces->onFirstPage() ? 'text-gray-400 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-100' }} px-3 py-2 rounded-md">
                ...
                @if ($startPage > 1)
                    <a href="{{ $provinces->appends(Request::except('page'))->url(1) }}"
                ...
                @foreach (range($startPage, $endPage) as $page)
                    <a href="{{ $provinces->appends(Request::except('page'))->url($page) }}"
                ...
                @if ($endPage < $lastPage)
                    @if ($endPage < $lastPage - 1)
                        <span class="px-2 py-2 text-gray-600">...</span>
                    @endif
                    <a href="{{ $provinces->appends(Request::except('page'))->url($lastPage) }}"
                       class="px-3 py-2 text-gray-600 bg-white hover:bg-gray-100 rounded-md">
                        {{ $lastPage }}
                    </a>
                @endif
                ...
                <!-- Next Page Link -->
                <a href="{{ $provinces->appends(Request::except('page'))->nextPageUrl() }}"
                   class="{{ !$provinces->hasMorePages() ? 'text-gray-400 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-100' }} px-3 py-2 rounded-md">
