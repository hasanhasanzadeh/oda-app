        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden mb-8">
            ...
            <div class="relative">
                ...
                @endif
                    ...
                    <div class="p-6 {{ $content->photo ? 'bg-gradient-to-t from-black/60 to-transparent absolute bottom-0 left-0 right-0 text-white' : 'bg-gray-50 dark:bg-gray-750 border-b border-gray-200 dark:border-gray-700' }}">
                    <div class="flex items-center justify-between">
                        <div>
                            <div class="flex items-center space-x-2 rtl:space-x-reverse mb-2">
                            <span class="{{ $content->status == 1 ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' }} text-xs font-semibold px-2.5 py-0.5 rounded">
                                {{ $content->status == 1 ? __('message.active') : __('message.inactive') }}
                            </span>
                                <span class="text-xs {{ $content->photo ? 'text-gray-200' : 'text-gray-500 dark:text-gray-400' }}">
                                ID: {{ $content->id }}
                            </span>
                            </div>
                            <h2 class="text-xl font-bold {{ $content->photo ? 'text-white' : 'text-gray-800 dark:text-gray-200' }}">
                                {{ $content->title }}
                            </h2>
                        ...
                        <div class="hidden md:block">
                            @if(config('app.locale')=='fa')
                                <span class="text-sm {{ $content->photo ? 'text-gray-200' : 'text-gray-500 dark:text-gray-400' }}">
                                {{ verta()->instance($content->created_at)->format('%d %B %Y') }}
                            </span>
                            @else
                                <span class="text-sm {{ $content->photo ? 'text-gray-200' : 'text-gray-500 dark:text-gray-400' }}">
            ...
            <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700 flex justify-end space-x-3 rtl:space-x-reverse">
                ...
                <form action="{{ route('pages.destroy', $content->id) }}" method="POST" class="inline-block"  id="delete-form-{{ $content->id }}">
                    @csrf
                    @method('DELETE')
