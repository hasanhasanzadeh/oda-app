...
@section('content')
    <div class="bg-white my-6 border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
        <div class="block md:flex items-start justify-between p-6 border-b lg:flex-row lg:items-center dark:border-gray-700">
            <div>
                <h2 class="text-lg font-bold text-gray-800 dark:text-white">{{ __('message.latest_products') }}</h2>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('message.product_list_description') }}</p>
            </div>
            ...
            <form method="get" id="myForm" class="block md:flex items-center mt-4 gap-4 space-x-reverse lg:mt-0">
                ...
                <div class="relative">
                    ...
                    <input type="text" id="searchInput" name="search" value="{{request('search')??''}}"
                           placeholder="{{ __('message.search_products') }}"
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tr class="bg-gray-50 dark:bg-gray-900 text-right">
                    ...
                    <x-sortable-column column="price" label="{{__('message.price')}}" :sort="$sort"
                                       :direction="$direction"/>
                    <x-sortable-column column="quantity" label="{{__('message.quantity')}}" :sort="$sort"
                                       :direction="$direction"/>
                ...
                @foreach($products as $index=>$product)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 hover:cursor-pointer">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="mr-2 font-medium text-gray-900 dark:text-white">{{ $products->firstItem() + $index }}</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <img src="{{$product->photo->address??asset('images/no-image.png')}}"
                                     class="w-20 h-20 ml-3 rounded shadow" alt="">
                                <div>
                                    <div class="text-md font-medium text-gray-900 dark:text-white">{{$product->name}}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            @else
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-pink-100 text-pink-800 dark:bg-pink-900 dark:bg-opacity-20 dark:text-pink-400">
                                <span class="w-1.5 h-1.5 mr-1.5 bg-pink-500 rounded-full"></span>
                                <span class="px-2">{{__('message.soon')}}</span>
                                </span>
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900 dark:text-white">{{verta($product->created_at)->format('d F Y')}}</div>
                            <div class="text-xs text-gray-500 dark:text-gray-400">{{verta($product->created_at)->format('h:i A')}}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <div class="flex items-center space-x-3 space-x-reverse">
                                ...
                                <div>
                                    <form action="{{route('products.destroy',$product->id)}}" method="post"
                                        ...
                                        @csrf
                                        {{method_field('DELETE')}}
                                        <button type="button" name="delete" onclick="confirmDelete({{ $product->id }})"
        ...
        <div class="flex flex-col md:flex-row items-center justify-between px-6 py-4 bg-white border-t dark:bg-gray-800 dark:border-gray-700">
            ...
            <div class="flex items-center justify-start w-full md:w-auto mb-4 md:mb-0 text-sm text-gray-600 dark:text-gray-400">
                <div>
                    {{ __('message.show_items') }} {{ $products->firstItem() }} {{ __('message.to') }} {{ $products->lastItem() }} {{ __('message.of') }} {{ $products->total() }} {{ __('message.items') }}
                </div>
            ...
            <div class="flex items-center space-x-1 space-x-reverse">
                ...
                <a href="{{ $products->appends(Request::except('page'))->previousPageUrl() }}"
                   class="{{ $products->onFirstPage() ? 'text-gray-400 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-100' }} px-3 py-2 rounded-md">
                ...
                @if ($startPage > 1)
                    <a href="{{ $products->appends(Request::except('page'))->url(1) }}"
                ...
                @if ($endPage < $lastPage)
                    @if ($endPage < $lastPage - 1)
                        <span class="px-2 py-2 text-gray-600">...</span>
                    @endif
                    <a href="{{ $products->appends(Request::except('page'))->url($lastPage) }}"
                       class="px-3 py-2 text-gray-600 bg-white hover:bg-gray-100 rounded-md">
                        {{ $lastPage }}
                    </a>
                @endif
                ...
                <a href="{{ $products->appends(Request::except('page'))->nextPageUrl() }}"
                   class="{{ !$products->hasMorePages() ? 'text-gray-400 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-100' }} px-3 py-2 rounded-md">
