...
@section('content')
    <div class="bg-white my-6 border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
        <div class="block md:flex items-start justify-between p-6 border-b lg:flex-row lg:items-center dark:border-gray-700">
                <div>
                    <h2 class="text-lg font-bold text-gray-800 dark:text-white">بازدید های اخیر</h2>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">لیست آخرین بازدید های ثبت شده در سیستم</p>
                </div>
            <div class="flex justify-end gap-2">
                <div>
                    <form action="{{route('visits.deleteAll')}}" method="post"
                          id="delete-form-all" class="inline">
                        @csrf
                        {{method_field('DELETE')}}
                        <button type="button" name="delete" onclick="confirmDeleteAll()"
                                class="block md:flex w-full my-4 items-center px-3 py-2 text-sm text-red-600 bg-red-100 rounded-lg dark:bg-red-700 dark:text-red-300"
                                title="{{__('message.delete')}}">
                            <i class="fas fa-trash"></i>
                            <span class="px-2">@lang('message.delete') @lang('message.all')</span>
                        </button>
                    </form>
                </div>
                <div>
                    <form method="get" id="myForm" class="block md:flex items-center mt-4 gap-4 space-x-reverse lg:mt-0">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="searchInput" name="search" value="{{request('search')??''}}"
                                   placeholder="جستجو در بازدید های ..."
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tr class="bg-gray-50 dark:bg-gray-900 text-right">
                    ...
                    <x-sortable-column column="url" label="{{__('message.url')}}" :sort="$sort"
                                       :direction="$direction"/>
                    <x-sortable-column column="role" label="{{__('message.role')}}" :sort="$sort"
                                       :direction="$direction"/>
                    <x-sortable-column column="score" label="{{__('message.view_count')}}" :sort="$sort"
                                       :direction="$direction"/>
                    <x-sortable-column column="ip_address" label="{{__('message.ip_address')}}" :sort="$sort"
                                       :direction="$direction"/>
                ...
                @foreach($visits as $index=>$visit)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 hover:cursor-pointer">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <input type="checkbox" name="city_{{$visit->id}}"
                                   class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600">
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="mr-2 font-medium text-gray-900 dark:text-white">{{ $visits->firstItem() + $index }}</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="px-2 text-gray-900 dark:text-white">{{$visit->url}}</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="px-2 text-gray-900 dark:text-white">
                                @if($visit->role!='gust' && $visit->user_id)
                                   <div class="flex flex-col items-center justify-center">
                                        <div>
                                            <a href="{{route('customers.show',$visit->user_id)}}" title="{{$visit->user->fullName}}">
                                                <img src="{{$visit->user->avatar->address??asset('images/user/avatar-profile.png')}}" class="h-12 w-12 rounded-full shadow" alt="">
                                            </a>
                                        </div>
                                        <div>
                                            <span class="py-2">{{$visit->role}}</span>
                                        </div>
                                   </div>
                                @else
                                    <div class="flex items-center justify-center">
                                        <div>
                                            <img src="{{asset('images/user/avatar-profile.png')}}" class="h-12 w-12 rounded-full shadow" alt="">
                                        </div>
                                        <div>
                                            <span class="py-2 text-gray-900 dark:text-white">{{$visit->role}}</span>
                                        </div>
                                   </div>
                                @endif
                            </span>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900 dark:bg-opacity-20 dark:text-green-400">
                                    ...
                                    <span class="px-2">{{$visit->score}}</span>
                                </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:bg-opacity-20 dark:text-blue-400">
                                    ...
                                    <span class="px-2">{{$visit->ip_address}}</span>
                                </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900 dark:text-white">{{verta($visit->created_at)->format('d F Y')}}</div>
                            <div class="text-xs text-gray-500 dark:text-gray-400">{{verta($visit->created_at)->format('h:i A')}}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <div class="flex items-center space-x-3 space-x-reverse">
                                <div>
                                    <form action="{{route('visits.destroy',$visit->id)}}" method="post"
                                        ...
                                        @csrf
                                        {{method_field('DELETE')}}
                                        <button type="button" name="delete" onclick="confirmDelete({{ $visit->id }})"
        ...
        <div class="flex flex-col md:flex-row items-center justify-between px-6 py-4 bg-white border-t dark:bg-gray-800 dark:border-gray-700">
            ...
            <div class="flex items-center justify-start w-full md:w-auto mb-4 md:mb-0 text-sm text-gray-600 dark:text-gray-400">
                <div>
                    نمایش {{ $visits->firstItem() }} تا {{ $visits->lastItem() }} از {{ $visits->total() }} مورد
                </div>
            ...
            <div class="flex items-center space-x-1 space-x-reverse">
                <!-- Previous Page Link -->
                <a href="{{ $visits->appends(Request::except('page'))->previousPageUrl() }}"
                   class="{{ $visits->onFirstPage() ? 'text-gray-400 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-100' }} px-3 py-2 rounded-md">
                ...
                @if ($startPage > 1)
                    <a href="{{ $visits->appends(Request::except('page'))->url(1) }}"
                ...
                @foreach (range($startPage, $endPage) as $page)
                    <a href="{{ $visits->appends(Request::except('page'))->url($page) }}"
                ...
                @if ($endPage < $lastPage)
                    @if ($endPage < $lastPage - 1)
                        <span class="px-2 py-2 text-gray-600">...</span>
                    @endif
                    <a href="{{ $visits->appends(Request::except('page'))->url($lastPage) }}"
                       class="px-3 py-2 text-gray-600 bg-white hover:bg-gray-100 rounded-md">
                        {{ $lastPage }}
                    </a>
                @endif
                ...
                <!-- Next Page Link -->
                <a href="{{ $visits->appends(Request::except('page'))->nextPageUrl() }}"
                   class="{{ !$visits->hasMorePages() ? 'text-gray-400 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-100' }} px-3 py-2 rounded-md">
