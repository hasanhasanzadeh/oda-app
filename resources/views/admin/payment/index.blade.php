...
@section('content')
    <div class="bg-white my-6 border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
        <div class="block md:flex items-start justify-between p-6 border-b lg:flex-row lg:items-center dark:border-gray-700">
            <div>
                <h2 class="text-lg font-bold text-gray-800 dark:text-white">پرداخت های اخیر</h2>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">لیست آخرین پرداخت های ثبت شده در سیستم</p>
            </div>
                    ...
                    <div x-show="filterOpen"
                        ...
                        <div class="p-4">
                            ...
                            <div class="space-y-2">
                                ...
                                <div>
                                    <label for="status" class="block mb-1 text-xs text-gray-600 dark:text-gray-400">وضعیت </label>
                                    <select name="status"
                                            ...
                                            id="status">
                                        <option value=""
                                                @if(!request('status') || request('status')==null) selected @endif>وضعیت
                                            را انتخاب کنید
                                        </option>
                                        <option value="done" @if(request('status')=='done') selected @endif>پرداخت شده
                                        </option>
                                        <option value="pending" @if(request('status')=='pending') selected @endif>در حال
                                            انجام
                                        </option>
                                        <option value="canceled" @if(request('status')=='canceled') selected @endif>
                                            پرداخت های کنسل شده
                                        </option>
                                        <option value="failed" @if(request('status')=='failed') selected @endif>پرداخت
                                            نشده
                                        </option>
                                    </select>
                                </div>
            ...
            <table class="w-full">
                ...
                <tr class="bg-gray-50 dark:bg-gray-900 text-right">
                    ...
                    <x-sortable-column column="full_name" label="{{__('message.full_name')}}" :sort="$sort"
                                       :direction="$direction"/>
                    <x-sortable-column column="paymentable_id" label="{{__('message.id')}}" :sort="$sort"
                                       :direction="$direction"/>
                    <x-sortable-column column="transaction_id" label="{{__('message.transaction_id')}}" :sort="$sort"
                                       :direction="$direction"/>
                    <x-sortable-column column="reference_id" label="{{__('message.reference_id')}}" :sort="$sort"
                                       :direction="$direction"/>
                ...
                @foreach($payments as $index=>$payment)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 hover:cursor-pointer">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <input type="checkbox" name="city_{{$payment->id}}"
                                   class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600">
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="mr-2 font-medium text-gray-900 dark:text-white">{{ $payments->firstItem() + $index }}</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-gray-900 dark:text-white">
                            <a href="{{route('customers.show',$payment->user_id)}}" title="نمایش اطلاعات کاربر">
        ...
        <div class="flex flex-col md:flex-row items-center justify-between px-6 py-4 bg-white border-t dark:bg-gray-800 dark:border-gray-700">
            ...
            <div class="flex items-center space-x-1 space-x-reverse">
                <!-- Previous Page Link -->
                <a href="{{ $payments->appends(Request::except('page'))->previousPageUrl() }}"
                   class="{{ $payments->onFirstPage() ? 'text-gray-400 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-100' }} px-3 py-2 rounded-md">
                ...
                @if ($startPage > 1)
                    <a href="{{ $payments->appends(Request::except('page'))->url(1) }}"
                ...
                @foreach (range($startPage, $endPage) as $page)
                    <a href="{{ $payments->appends(Request::except('page'))->url($page) }}"
                ...
                @if ($endPage < $lastPage)
                    @if ($endPage < $lastPage - 1)
                        <span class="px-2 py-2 text-gray-600">...</span>
                    @endif
                    <a href="{{ $payments->appends(Request::except('page'))->url($lastPage) }}"
                       class="px-3 py-2 text-gray-600 bg-white hover:bg-gray-100 rounded-md">
                        {{ $lastPage }}
                    </a>
                @endif
                ...
                <!-- Next Page Link -->
                <a href="{{ $payments->appends(Request::except('page'))->nextPageUrl() }}"
                   class="{{ !$payments->hasMorePages() ? 'text-gray-400 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-100' }} px-3 py-2 rounded-md">
