...
@section('content')
    <div class="bg-white my-6 border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
        <div class="block md:flex items-start justify-between p-6 border-b lg:flex-row lg:items-center dark:border-gray-700">
            <div>
                <h2 class="text-lg font-bold text-gray-800 dark:text-white">نقش های اخیر</h2>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">لیست آخرین نقش های ثبت شده در سیستم</p>
            </div>
            ...
            <table class="w-full">
                ...
                <tr class="bg-gray-50 dark:bg-gray-900 text-right">
                    ...
                    <x-sortable-column column="name" label="{{__('message.name')}}" :sort="$sort"
                                       :direction="$direction"/>
                    <x-sortable-column column="display_name" label="{{__('message.display_name')}}" :sort="$sort"
                                       :direction="$direction"/>
                    <x-sortable-column column="guard_name" label="{{__('message.guard_name')}}" :sort="$sort"
                                       :direction="$direction"/>
                ...
                @foreach($roles as $index=>$role)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 hover:cursor-pointer">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="mr-2 font-medium text-gray-900 dark:text-white">{{ $roles->firstItem() + $index }}</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-md font-medium text-gray-900 dark:text-white">{{$role->name}}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-md font-medium text-gray-900 dark:text-white">{{$role->display_name}}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-md font-medium text-gray-900 dark:text-white">{{$role->guard_name}}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <div class="flex items-center space-x-3 space-x-reverse">
                                ...
                                <div>
                                    <form action="{{route('roles.destroy',$role->id)}}" method="post"
                                        ...
                                        @csrf
                                        {{method_field('DELETE')}}
                                        <button type="button" name="delete" onclick="confirmDelete({{ $role->id }})"
        ...
        <div class="flex flex-col md:flex-row items-center justify-between px-6 py-4 bg-white border-t dark:bg-gray-800 dark:border-gray-700">
            ...
            <div class="flex items-center justify-start w-full md:w-auto mb-4 md:mb-0 text-sm text-gray-600 dark:text-gray-400">
                <div>
                    نمایش {{ $roles->firstItem() }} تا {{ $roles->lastItem() }} از {{ $roles->total() }} مورد
                </div>
            ...
            <div class="flex items-center space-x-1 space-x-reverse">
                ...
                <a href="{{ $roles->appends(Request::except('page'))->previousPageUrl() }}"
                   class="{{ $roles->onFirstPage() ? 'text-gray-400 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-100' }} px-3 py-2 rounded-md">
                ...
                @if ($startPage > 1)
                    <a href="{{ $roles->appends(Request::except('page'))->url(1) }}"
                ...
                @if ($endPage < $lastPage)
                    @if ($endPage < $lastPage - 1)
                        <span class="px-2 py-2 text-gray-600">...</span>
                    @endif
                    <a href="{{ $roles->appends(Request::except('page'))->url($lastPage) }}"
                       class="px-3 py-2 text-gray-600 bg-white hover:bg-gray-100 rounded-md">
                        {{ $lastPage }}
                    </a>
                @endif
                ...
                <a href="{{ $roles->appends(Request::except('page'))->nextPageUrl() }}"
                   class="{{ !$roles->hasMorePages() ? 'text-gray-400 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-100' }} px-3 py-2 rounded-md">
